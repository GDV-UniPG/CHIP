export function openRouteservice() {
	...
	L.Routing.OpenRouteServiceV2 = L.Class.extend({
         ...
         */
        options: {
            serviceUrl: 'http://localhost:8082/ors/v2/directions',
            profile: 'driving-car'
        },
        ...
        buildRouteUrl: function(waypoints) {
              ...
              return {
                  url: `${this.options.serviceUrl}/${this.options.profile}`,
                  body: body 
              };
          },
		route: function (waypoints, callback, context) {
			...
			waypoints.forEach(element => {
                ...
                const { url, body } = this.buildRouteUrl(waypoints);
                    ...
                    if (request.readyState === 4) {
                        if (request.status >= 200 && request.status < 300) {
                            const response = JSON.parse(request.responseText);
								...
								this._routeDone(response, wps, callback, context);
                        }
                    }
                }
				request.send(body);
